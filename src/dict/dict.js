        ...
        for(let i=0;i<dtAr.length;i++){
            ...
            if ($.inArray(key,keyAr)!==-1) {
                if (this.dict[key][lang]) {
                    if(item.isEntity)
                        item.setAttribute('text','value',urlencode.decode(this.dict[key][lang]));
                    if($(item).text())
                        $(item).text(urlencode.decode(this.dict[key][lang]));
                    if($(item).attr("title"))
                        $(item).attr("title", urlencode.decode(this.dict[key][lang]));
                    if($(item).attr("placeholder"))
                        $(item).attr("placeholder", urlencode.decode(this.dict[key][lang]));
                    if($(item).attr("value"))
                        $(item).attr("value", urlencode.decode(this.dict[key][lang]));
                }
            }
        }
    }
