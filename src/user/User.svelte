...
<script>
  ...
  import { msg } from "./rtc/RTCUser.js";
  ...
  $: if (status) 
   ...
   import {hosts} from './rtc/stores.js'  

  (async ()=>{
    
    let host = (await (await fetch('./assets/host.json')).json());
    hosts.set(host)

    let dict = (await (await fetch(host.host + './assets/dict/dict.json')).json());
    dicts.set( dict);
